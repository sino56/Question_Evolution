...
3. 对每个可能相关的算子检查卡片 required_slots：结合 evolution_mode、authorization_checked、authorization_id 和槽位审计，区分来源已满足、可在已登记题内假设世界中合成、以及不可合成的缺失槽位。不得把真实外部事实、真实规则、真实专业阈值或真实案件结论补进题面。
4. 解决近邻重叠：同一失败机制只保留最具体、最直接且构题闭包更小的算子；只有存在独立真实失败机制时才并列选择多个算子。

候选门禁：
- operator_candidates 只能包含同时满足“直接压测目标失败机制或另一明确真实失败机制 + 所有槽位均为来源已满足或当前模式允许的题内受控合成 + 无需伪造真实外部材料”的算子。
- 主题、对象或业务词相似不是硬槽位满足。source_faithful 下缺路径节点/边/端点时间窗时不得选择路径算子；缺竞争实体及其跨节点绑定事实时不得选择实体绑定算子。受控假设模式可合成的局部实体、时间窗、路径、题内规则或参数必须登记为题内设定，不能冒充来源事实。
- 任何不可合成硬槽位缺失、只适合作人工复盘、或与已选算子重复压测同一失败机制的方向，都不得进入 operator_candidates。题内规则、局部参数、竞争解释、相关干扰事实和中性业务判断问法本身不是拒绝理由。
