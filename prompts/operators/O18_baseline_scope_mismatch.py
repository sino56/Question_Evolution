...
SPEC = OperatorPromptSpec(
    ...
    question_construction="并列给出基线纳入条件、必要摘要和当前观测，避免用来源权威性或叙述完整度偏向其中一个基线。",
    ...
    hidden_role_balance_controls=(
        "两个基线使用相近语义负载、统计粒度和来源可信度",
        "正确基线不独占更精确数字或更完整摘要",
    ),
    semantic_economy=(
        "当前观测和共享业务背景只写一次，候选基线只呈现纳入口径与必要统计摘要的差异。",
        "保留判断可比性和异常性所需输入，不复制完整样本说明或统计表。",
        "不得让适用基线独占更完整摘要、权威措辞或异常性结论。",
    ),
