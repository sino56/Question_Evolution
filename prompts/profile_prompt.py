...
2. 判断候选答案是否属于高分虚高，或低分/中间分样本中是否暴露了可继续探测的真实边界价值。
3. 高分虚高时，说明最主要虚高原因；低分或中间分边界样本时，说明它暴露的唯一主判断错误。
4. 优先识别当前弱模型仍可能不稳的近似业务判断竞争，不要默认复用普通两层拆分、显式最小缺口或常规线索/结论区分。
...
`candidate_overscore_cause` 优先使用：近似业务表述层级混淆、不可见区间状态补设、强线索替代未闭合门槛、新增事实后原评价保留范围错误、题面信息闭包越界、单变量变化后整体保留或撤回、正常解释导致风险过度降级、处置触发与事实定性混淆、基线样本范围错配、反常线索主线切换失败、评分噪声、基础边界判断过稳。
...

{
  ...
  "overscore_diagnosis": {
    ...
    "target_failure_mode": "把足以触发核查的异常表述成已经证实的事实",
    "why_high_score_is_suspicious": "候选答案的处置建议可执行，但报告表述越过了题面能够支持的事实范围"
  }
}
""".strip()
