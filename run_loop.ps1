...
$SEARCH_SINGLE_OPERATOR_BOUNDARY_TARGET = Get-EnvOrDefault "SEARCH_SINGLE_OPERATOR_BOUNDARY_TARGET" $SEARCH_BOUNDARY_TARGET
$SEARCH_STACKED_OPERATOR_BOUNDARY_TARGET = Get-EnvOrDefault "SEARCH_STACKED_OPERATOR_BOUNDARY_TARGET" $SEARCH_BOUNDARY_TARGET
$searchTotalBoundaryDefault = ([int]$SEARCH_SINGLE_OPERATOR_BOUNDARY_TARGET + [int]$SEARCH_STACKED_OPERATOR_BOUNDARY_TARGET).ToString()
$SEARCH_TOTAL_BOUNDARY_HARD_CAP = Get-EnvOrDefault "SEARCH_TOTAL_BOUNDARY_HARD_CAP" $searchTotalBoundaryDefault
    ...
    if (-not (Test-PublishedArtifact $roundScored "scoring" (Join-Path $roundDir "rubric.jsonl"))) {
        ...
        if ($SEARCH_MODE -in @("multi_operator_branch", "multi_operator_vertical_stack")) {
            ...
            if ($SEARCH_MODE -eq "multi_operator_vertical_stack") {
                $searchArgs = @("vertical_operator_search.py", "--input", $routed, "--output", $searchStateOutput, "--work-dir", (Join-Path $roundDir "search"), "--memory-dir", $MEMORY_DIR, "--branch-window", $SEARCH_BRANCH_WINDOW, "--boundary-target", $SEARCH_BOUNDARY_TARGET, "--single-operator-boundary-target", $SEARCH_SINGLE_OPERATOR_BOUNDARY_TARGET, "--stacked-operator-boundary-target", $SEARCH_STACKED_OPERATOR_BOUNDARY_TARGET, "--total-boundary-hard-cap", $SEARCH_TOTAL_BOUNDARY_HARD_CAP, "--max-depth", $SEARCH_MAX_DEPTH, "--pipeline-mode", $SEARCH_PIPELINE_MODE, "--artifact-retention", $SEARCH_ARTIFACT_RETENTION, "--max-request-attempts-per-sample", $SEARCH_MAX_REQUEST_ATTEMPTS_PER_SAMPLE, "--max-evaluations-per-sample", $SEARCH_MAX_EVALUATIONS_PER_SAMPLE, "--sample-timeout-seconds", $SEARCH_SAMPLE_TIMEOUT_SECONDS, "--routing-mode", $ROUTING_MODE, "--router-model", $ROUTER_MODEL, "--router-base-url", $ROUTER_BASE_URL, "--router-concurrency", $ROUTER_CONCURRENCY, "--router-timeout", $ROUTER_TIMEOUT, "--router-retries", $ROUTER_RETRIES, "--router-cache", (Join-Path $EXP_DIR "router_cache.jsonl"), "--profile-model", $PROFILE_MODEL, "--profile-base-url", $PROFILE_BASE_URL, "--profile-concurrency", $PROFILE_CONCURRENCY)
        ...
        $rebuiltReference = Join-Path $roundDir "rebuilt_reference.jsonl"
        Invoke-Step $rebuiltReference "rebuild_reference_answer" $evolved "[Round $round] Step 9/14: rebuild_reference_answer.py" { Invoke-Python "rebuild_reference_answer.py" "--input" $evolved "--output" $rebuiltReference "--model" $GPT_MODEL "--base-url" $ANSWER_BASE_URL "--execution-scope" "full_iteration" }
        Invoke-Step $rubric "gen_rubric" $rebuiltReference "[Round $round] Step 10/14: gen_rubric.py" { Invoke-Python "gen_rubric.py" "--input" $rebuiltReference "--output" $rubric "--concurrency" $RUBRIC_CONCURRENCY "--model" $GPT_MODEL "--base-url" $RUBRIC_BASE_URL "--performance-events" (Join-Path $roundDir "performance_events.jsonl") }
        Invoke-Step $withAnswers "collect_answers" $rubric "[Round $round] Step 11/14: collect_answers.py (weak model; final prompt only)" { Invoke-Python "collect_answers.py" "--input" $rubric "--output" $withAnswers "--concurrency" $ANSWER_CONCURRENCY "--request-concurrency" $ANSWER_REQUEST_CONCURRENCY "--samples" "1" "--model" $QWEN_MODEL "--base-url" $QWEN_BASE_URL "--performance-events" (Join-Path $roundDir "performance_events.jsonl") }
        $scoringInput = $withAnswers
        $scoringMode = "complete"
        if ($DEFER_GPT_EXPERIMENTAL_EVALUATION -eq "true") {
            $decisionScored = Join-Path $roundDir "decision_scored.jsonl"
            Invoke-Step $decisionScored "scoring_decision" $withAnswers "[Round $round] Step 12a/14: Qwen decision checkpoint" { Invoke-Python "scoring.py" "--input" $withAnswers "--output" $decisionScored "--evaluation-mode" "decision" "--answer-mode" "llm" "--answer-base-url" $QWEN_BASE_URL "--answer-api-key" $QWEN_API_KEY "--answer-model" $QWEN_MODEL "--judge-base-url" $QWEN_BASE_URL "--judge-api-key" $QWEN_API_KEY "--judge-model" $QWEN_MODEL "--answer-trials" $SCORING_ANSWER_TRIALS "--gpt-answer-trials" $GPT_ANSWER_TRIALS "--qwen-judge-repeats" $QWEN_JUDGE_REPEATS "--gpt-judge-base-url" $GPT_JUDGE_BASE_URL "--gpt-judge-api-key" $GPT_JUDGE_API_KEY "--gpt-judge-model" $GPT_JUDGE_MODEL "--gpt-judge-repeats" $GPT_JUDGE_REPEATS "--gpt-answer-base-url" $GPT_ANSWER_BASE_URL "--gpt-answer-api-key" $GPT_ANSWER_API_KEY "--gpt-answer-model" $GPT_ANSWER_MODEL "--qwen-max-concurrent" $QWEN_SCORING_MAX_CONCURRENT "--gpt-max-concurrent" $GPT_SCORING_MAX_CONCURRENT "--concurrency" $SCORING_CONCURRENCY "--performance-events" (Join-Path $roundDir "performance_events.jsonl") }
            $scoringInput = $decisionScored
            $scoringMode = "experimental"
        }
        Invoke-Step $roundScored "scoring" $scoringInput "[Round $round] Step 12/14: scoring.py complete scored artifact" { Invoke-Python "scoring.py" "--input" $scoringInput "--output" $roundScored "--evaluation-mode" $scoringMode "--answer-mode" "llm" "--answer-base-url" $QWEN_BASE_URL "--answer-api-key" $QWEN_API_KEY "--answer-model" $QWEN_MODEL "--judge-base-url" $QWEN_BASE_URL "--judge-api-key" $QWEN_API_KEY "--judge-model" $QWEN_MODEL "--answer-trials" $SCORING_ANSWER_TRIALS "--gpt-answer-trials" $GPT_ANSWER_TRIALS "--qwen-judge-repeats" $QWEN_JUDGE_REPEATS "--gpt-judge-base-url" $GPT_JUDGE_BASE_URL "--gpt-judge-api-key" $GPT_JUDGE_API_KEY "--gpt-judge-model" $GPT_JUDGE_MODEL "--gpt-judge-repeats" $GPT_JUDGE_REPEATS "--gpt-answer-base-url" $GPT_ANSWER_BASE_URL "--gpt-answer-api-key" $GPT_ANSWER_API_KEY "--gpt-answer-model" $GPT_ANSWER_MODEL "--qwen-max-concurrent" $QWEN_SCORING_MAX_CONCURRENT "--gpt-max-concurrent" $GPT_SCORING_MAX_CONCURRENT "--concurrency" $SCORING_CONCURRENCY "--performance-events" (Join-Path $roundDir "performance_events.jsonl") }
    }
    else {
